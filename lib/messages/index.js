...
var BufferUtil = primeaicore.util.buffer;
var Hash = primeaicore.crypto.Hash;
var $ = primeaicore.util.preconditions;

/**
 * A factory to build primeaicoin protocol messages.
...
function Messages(options) {
  ...
  this.network = options.network || primeaicore.Networks.defaultNetwork;
}
...
Messages.prototype._buildFromBuffer = function(command, payload) {
  if (!this.builder.commands[command]) {
    if (this.builder.unsupportedCommands.indexOf(command) > -1) {
      return; // ignore unsupported message commands
    }
